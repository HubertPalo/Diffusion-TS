...
def __main__(dataset_name = "UCI", preffix = "DAGHAR_UCI", num_checkpoints = 10, max_epochs = 12000, class_id = None, milestone = 10, scaler = "standard"):
    ...
    if class_id is not None:
        ...
        ori_data = np.transpose(train_x[train_y == class_id], (0, 2, 1))
    else:
        # Use all data
        data_x = train_x_scaled
        ori_data = np.transpose(train_x, (0, 2, 1))
    ...
    ori_data_samples = random.sample(list(ori_data), 20)
    fake_data_samples = random.sample(list(fake_data_unnormalized), 20)

    
    # Visualize the original data samples
    visualize(ori_data_samples, "Original Data Samples", f"{folder}_original_samples", parent_folder=f"{preffix}/{milestone}", ylim=(-8,8))
    visualize(ori_data_samples, "Original Data Samples", f"{folder}_original_samples", parent_folder=f"{preffix}/{milestone}", ylim=None)
    # Visualize the fake data samples
    visualize(fake_data_samples, "Fake Data Samples", f"{folder}_synthetic_samples", parent_folder=f"{preffix}/{milestone}", ylim=(-8,8))
    visualize(fake_data_samples, "Fake Data Samples", f"{folder}_synthetic_samples", parent_folder=f"{preffix}/{milestone}", ylim=None)

    # Visualize the original and fake data in 2D
    ori_data_tsne, fake_data_tsne = data_to_2d(train_x.transpose(0,2,1), fake_data_unnormalized)
    ...
    ori_data_fft = np.abs(np.fft.fft(train_x.transpose(0,2,1), axis=1))#.real
    fake_data_fft = np.abs(np.fft.fft(fake_data_unnormalized, axis=1))#.real
